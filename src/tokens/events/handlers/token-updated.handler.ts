...
export class TokenUpdatedHandler
  implements IEventHandler<TokenUpdatedEvent> {
  ...
  handle(event: TokenUpdatedEvent) {
    ...
    const tokenDto = event.tokenDto[0];
    this.repository.update(tokenDto.tokenId, tokenDto);
  }
}

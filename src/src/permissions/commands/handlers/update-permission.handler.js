...
let UpdatePermissionHandler = class UpdatePermissionHandler {
    ...
    execute(command) {
        return __awaiter(this, void 0, void 0, function* () {
            common_1.Logger.log('Async UpdatePermissionHandler...', 'UpdatePermissionCommand');
            const { streamId, permissionDto } = command;
            const permission = this.publisher.mergeObjectContext(yield this.repository.updatePermission(streamId, permissionDto));
            permission.commit();
        });
    }
};
UpdatePermissionHandler = __decorate([
    cqrs_1.CommandHandler(update_permission_command_1.UpdatePermissionCommand),

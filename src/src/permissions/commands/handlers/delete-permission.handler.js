...
let DeletePermissionHandler = class DeletePermissionHandler {
    ...
    execute(command) {
        return __awaiter(this, void 0, void 0, function* () {
            common_1.Logger.log('Async DeletePermissionHandler...', 'DeletePermissionCommand');
            const { streamId, permissionIdDto } = command;
            const permission = this.publisher.mergeObjectContext(yield this.repository.deletePermission(streamId, permissionIdDto._id));
            permission.commit();
        });
    }
};
DeletePermissionHandler = __decorate([
    cqrs_1.CommandHandler(delete_permission_command_1.DeletePermissionCommand),
